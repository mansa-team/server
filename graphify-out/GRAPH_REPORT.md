...
## Corpus Check
- 48 files · ~26,022 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 569 nodes · 807 edges · 45 communities (31 shown, 14 thin omitted)
- Extraction: 73% EXTRACTED · 27% INFERRED · 0% AMBIGUOUS · INFERRED: 214 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `017a08e7`
...
- `TestTickerIndex` --uses--> `StocksCacheManager`  [INFERRED]
...
Cohesion: 0.13
Nodes (11): Different query parameters should not share cache, Different query parameters should not share cache, Cache should expire after TTL, Cache should expire after TTL, Tests for query result caching, Tests for query result caching, CacheManager should track cached queries, CacheManager should track cached queries (+3 more)
...
Cohesion: 0.2
Nodes (4): generateKey(), FastAPI, createKey(), generateSecureKey()
...
- **Why does `FastAPI` connect `Responses` to `Stocks API Controller`, `Session Management`, `Scraper B3`, `Auth Service`, `API Key Model`, `MySQL Connectivity`, `Auth Constants`, `SSO`?**
...
- **Why does `StocksQueryManager` connect `Authentication Manager` to `SQLAlchemy Models`, `Device Detection`, `User Service`, `Database Utils`, `Validators`, `Responses`, `Enums`, `Prometheus Model`?**
