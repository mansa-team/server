...
## Corpus Check
- 47 files · ~25,618 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 475 nodes · 666 edges · 40 communities (28 shown, 12 thin omitted)
- Extraction: 74% EXTRACTED · 26% INFERRED · 0% AMBIGUOUS · INFERRED: 174 edges (avg confidence: 0.71)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ef8eac9a`
...
1. `StocksCacheManager` - 29 edges
2. `UserSession` - 26 edges
3. `StocksAPIKey` - 24 edges
4. `User` - 24 edges
5. `TestRoles` - 18 edges
6. `StocksQueryManager` - 17 edges
...
Cohesion: 0.08
Nodes (11): chat(), createSession(), getHistory(), generateKey(), FastAPI, lifespan(), createKey(), generateSecureKey() (+3 more)
...
### Community 19 - "Enums"
Cohesion: 0.18
Nodes (8): Tests for connection pool configuration, Stocks engine should have optimized pool settings, Performance tests for query operations, Query should respond within 1 second, Performance tests for query operations, Query should respond within 1 second, TestConnectionPool, TestQueryPerformance

### Community 20 - "Prometheus Model"
Cohesion: 0.31
Nodes (4): StocksQueryManager, Tests for lazy JSON deserialization, Query manager should have deserialize method, TestLazyDeserialization
...
## Knowledge Gaps
- **64 isolated node(s):** `Initial migration  Revision ID: 25af7ad931e7 Revises:  Create Date: 2026-04-`, `User sessions table  Revision ID: 4a2f1c9e3b5d Revises: 25af7ad931e7 Create Date`, `Tests for dynamic ticker index feature`, `Ticker index should be built when cache is loaded`, `Ticker index should contain all tickers from cache` (+59 more)
...
- **Why does `FastAPI` connect `Session Management` to `Stocks API Controller`, `Stocks Cache & Query`, `Scraper B3`, `Prometheus Chat`, `Scraper Service`, `Auth Service`?**
  _High betweenness centrality (0.359) - this node is a cross-community bridge._
- **Why does `Pytest Testing` connect `Authentication Manager` to `Stocks API Controller`, `Prometheus Chat`, `Stocks Cache & Query`, `Enums`?**
  _High betweenness centrality (0.223) - this node is a cross-community bridge._
- **Why does `Config` connect `Roles & Permissions` to `Auth Service`, `User Controller`, `Configuration & Logging`?**
  _High betweenness centrality (0.183) - this node is a cross-community bridge._
- **Are the 24 inferred relationships involving `StocksCacheManager` (e.g. with `StocksQueryManager` and `TestTickerIndex`) actually correct?**
  _`StocksCacheManager` has 24 INFERRED edges - model-reasoned connections that need verification._
