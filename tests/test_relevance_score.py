...
class _FakeMemory:
    def __init__(self, baseScore=7.0, lastAccessedAt=None):
...
class TestTimeDecay:
    def test_just_accessed_has_full_score(self):
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW)
        score = getRelevanceScore(mem, NOW)
        assert score == pytest.approx(1.0, abs=0.01)

    def test_one_day_old(self):
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=1))
        score = getRelevanceScore(mem, NOW)
        # R = e^(-1/7) ≈ 0.867
        assert score == pytest.approx(0.867, abs=0.05)

    def test_seven_days_old(self):
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=7))
        score = getRelevanceScore(mem, NOW)
        # R = e^(-7/7) = e^(-1) ≈ 0.368
        assert score == pytest.approx(0.368, abs=0.05)

    def test_thirty_days_old(self):
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=30))
        score = getRelevanceScore(mem, NOW)
        # R = e^(-30/7) ≈ 0.013
        assert score == pytest.approx(0.013, abs=0.01)

    def test_ninety_days_old(self):
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=90))
        score = getRelevanceScore(mem, NOW)
        # R = e^(-90/7) ≈ 3.3e-6, essentially 0
        assert score < 0.01

    def test_one_year_old(self):
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=365))
        score = getRelevanceScore(mem, NOW)
        assert score < 0.001

    def test_score_always_positive(self):
        for days in [0, 1, 7, 30, 90, 365, 730]:
            mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=days))
            score = getRelevanceScore(mem, NOW)
            assert score > 0
...
class TestStabilityImpact:
    def test_high_stability_beats_low_same_age(self):
        ...
        high = _FakeMemory(baseScore=14.0, lastAccessedAt=old)
        low = _FakeMemory(baseScore=3.0, lastAccessedAt=old)
    ...
    def test_sticky_memory_outlasts_ephemeral(self):
        ...
        sticky_4d = _FakeMemory(baseScore=14.0, lastAccessedAt=NOW - timedelta(days=4))
        ephemeral_1d = _FakeMemory(baseScore=3.0, lastAccessedAt=NOW - timedelta(days=1))
    ...
    def test_ephemeral_old_loses_to_sticky_new(self):
        ...
        ephemeral_old = _FakeMemory(baseScore=3.0, lastAccessedAt=NOW - timedelta(days=30))
        sticky_new = _FakeMemory(baseScore=14.0, lastAccessedAt=NOW)
    ...
    def test_higher_stability_always_wins_at_same_age(self):
        ...
        a = _FakeMemory(baseScore=3.0, lastAccessedAt=t)
        b = _FakeMemory(baseScore=14.0, lastAccessedAt=t)
...
class TestDecayMonotonicity:
    ...
    def test_never_accessed_uses_stability(self):
        """A memory with no lastAccessedAt uses baseScore as stability."""
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=None)
        score = getRelevanceScore(mem, NOW)
...
class TestNoCronNeeded:
    def test_same_memory_same_time_same_score(self):
        ...
        mem = _FakeMemory(baseScore=7.0, lastAccessedAt=NOW - timedelta(days=14))
