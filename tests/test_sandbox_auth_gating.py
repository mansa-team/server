...
class TestSandboxAuthGating:
    def test_urlSetEmptyTokenRaises(self):
        with patch.object(sandboxModule.Config.PROMETHEUS, "FORGEVM_URL", "http://forgevm:7423"), patch.object(
            sandboxModule.Config.PROMETHEUS, "FORGEVM_API_TOKEN", ""
        ):
    ...
    def test_urlSetWithTokenPassesThrough(self):
        with patch.object(sandboxModule.Config.PROMETHEUS, "FORGEVM_URL", "http://forgevm:7423"), patch.object(
            sandboxModule.Config.PROMETHEUS, "FORGEVM_API_TOKEN", "secret"
    ...
    def test_urlUnsetReturnsClient(self):
        with patch.object(sandboxModule.Config.PROMETHEUS, "FORGEVM_URL", ""), patch.object(
            sandboxModule.Config.PROMETHEUS, "FORGEVM_API_TOKEN", ""
        ), patch.object(sandboxModule, "AsyncClient") as mockClient:
            client = sandboxModule.getClient()
